...
import {LoanBaseMinimal, ILoanCoordinator, IERC20} from "../../loans/loanTypes/LoanBaseMinimal.sol";
...
contract NftfiRefinancingAdapter is IRefinancingAdapter {
    ...
    function getBorrowerAddress(
        ...
        ILoanCoordinator coordinator = _getCoordinator(_loanContract);
    ...
    function payOffRefinancable(
        ...
        IERC20(_payBackToken).approve(_getERC20TransferManagerAddress(_loanContract), _payBackAmount);
        LoanBaseMinimal(_loanContract).payBackLoan(uint32(_loanIdentifier));
        return (true);
    }
    ...
    function getCollateral(
        ...
        //    uint16 orginatioinFeeInBasisPoints;
        ...
        LoanData.LoanTerms memory lt = LoanBaseMinimal(_loanContract).getLoanTerms(uint32(_loanIdentifier));
        return (lt.nftCollateralContract, lt.nftCollateralId);
    }
    ...
    function getPayoffDetails(
        ...
        LoanData.LoanTerms memory lt = LoanBaseMinimal(_loanContract).getLoanTerms(uint32(_loanIdentifier));
        return (lt.loanERC20Denomination, lt.maximumRepaymentAmount);
    }
     ...
     * @return The loan coordinator contract.
     */
    function _getCoordinator(address _loanContract) private view returns (ILoanCoordinator) {
        LoanBaseMinimal loan = LoanBaseMinimal(_loanContract);
        return ILoanCoordinator(loan.hub().getContract(loan.LOAN_COORDINATOR()));
    }
     ...
     * @return The loan coordinator contract.
     */
    function _getERC20TransferManagerAddress(address _loanContract) private view returns (address) {
        LoanBaseMinimal loan = LoanBaseMinimal(_loanContract);
        return loan.getERC20TransferManagerAddress();
    }
}
