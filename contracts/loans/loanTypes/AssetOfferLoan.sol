...
contract AssetOfferLoan is LoanBaseMinimal {
     ...
     * @param _offer - The offer made by the lender.
     */
    function _loanSanityChecksOffer(LoanData.Offer memory _offer) internal view virtual {
        ...
        if (
            (_offer.allowedBorrowers[0] != address(msg.sender) &&
                hub.getContract(ContractKeyUtils.getIdFromStringKey("REFINANCING")) != address(msg.sender))
        ) {
